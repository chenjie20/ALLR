...
for idx = 1 : n
    sample_data(:, idx) = sample_data(:, idx) ./ max(1e-12,  norm(sample_data(:, idx)));
end
    ...
    for apla_idx = 1 : apla_len
        ...
        for ratio_idx = 1 : ratio_len
            ...
            for mu_idx = 1 : mu_len
                ...
                std_acc = std(results);
                disp([ratio lambda apla avg_acc std_acc]);
                dlmwrite('classification_allr_yaleb.txt', [ratio lambda apla options.alpha avg_acc std_acc], '-append', 'delimiter', '\t', 'newline', 'pc');
            end
        end
    end
end